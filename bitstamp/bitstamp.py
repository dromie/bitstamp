...
# 2. Don't want all the ballast that comes along (a whole lib for three channels and supporting libs)
    ...
    self.ws_channels_templates = {
    WS_CHANNEL_LIVE_TRADES: '{"event":"pusher:subscribe","data":{"channel":"live_trades%s"}}',
    WS_CHANNEL_ORDER_BOOK: '{"event":"pusher:subscribe","data":{"channel":"order_book%s"}}',
    WS_CHANNEL_ORDER_BOOK_DIFF: '{"event":"pusher:subscribe","data":{"channel":"diff_order_book%s"}}',
    }
    self.ws_data_events = ['data', 'trade']

  def ws_open_message(self, channel, trade_pair=None):
    postfix=""
    if trade_pair is not None: 
      postfix="_%s" % trade_pair
    return self.ws_channels_templates[channel] % postfix


  def __str__(self):
    return "Bitstamp public client"
  ...
  def trading_pairs(self):
    resource = 'v2/trading-pairs-info/'
    response = requests.get('{}{}'.format(self.api_endpoint, resource))
    return json.loads(response.text)

  def __on_open(self, channel, trading_pair=None):
    channel_string = self.ws_open_message(channel, trading_pair)
  ...
  def attach_ws(self, channel, callback, trading_pair=None, error_callback=None, close_callback=None):
    ...
    self.ws.on_open = self.__on_open(channel, trading_pair)
    self.ws.run_forever()
...
# If still not api key and no secret, raise
  ...
  def __str__(self):
    ...
    :return: None
    '''
    return json.dumps(["Bitstamp private",self.api_key, self.secret, self.customer_id])
